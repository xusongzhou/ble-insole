...
public class ClipImageLayout extends RelativeLayout {
    ...
    public ClipImageLayout(Context context, AttributeSet attrs) {
        ...
        mClipImageView = new ClipImageBorderView(getContext());
	}
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (first) {
            ...
            addView(mZoomImageView, width, height);
            addView(mClipImageView, width, height);
            // 计算padding的px
    ...
    private class ClipImageBorderView extends View {
        /**
         * 水平方向与View的边距
         */
        ...
        private Paint mPaintCirle2;// 圆（实心圆）paint
        private Paint mPaintRect;// 阴影层paint
        private Paint clearFbPaint;//清除前景蒙版层画布内容
        ...
        public ClipImageBorderView(Context context, AttributeSet attrs, int defStyle) {
            ...
            mPaintCirle2.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.XOR));// XOR模式：重叠部分被掏空
            ...
            clearFbPaint = new Paint();
            clearFbPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            mPaintCirle.setStrokeWidth((width - 2 * mHorizontalPadding) / 2); // 实心圆半径
            mPaintCirle2.setStrokeWidth((width - 2 * mHorizontalPadding) / 2); // 实心圆半径
            mCanvas.drawPaint(clearFbPaint);
            // 绘制阴影层
            mCanvas.drawRect(mRect, mPaintRect);
            // 绘制实心圆 ，绘制完后，在mCanvas画布中，mPaintRect和mPaintCirle相交部分即被掏空
            mCanvas.drawCircle(width / 2, height / 2, width / 2 - mHorizontalPadding, mPaintCirle);
            mCanvas.drawCircle(width / 2, height / 2, width / 2 - mHorizontalPadding, mPaintCirle2);

            // 将阴影层画进本View的画布中
            canvas.drawBitmap(mBgBitmap, null, mRect, mPaint);
            // 绘制圆环
    ...
    private class ClipZoomImageView extends ImageView implements
        ...
        public void onGlobalLayout() {
            if (once) {
                ...
                if (dw <= width - mHorizontalPadding * 2
                        && dh >= height - mVerticalPadding * 2) {// 如果图片的宽度<截图区宽度，并且图片高度>截图区高度时
                    scale = (width * 1.0f - mHorizontalPadding * 2) / dw;
                }

                if (dh <= height - mVerticalPadding * 2
                        && dw >= width - mHorizontalPadding * 2) {// 如果图片的高度<截图区高度，并且图片宽度>截图区宽度时
                    scale = (height * 1.0f - mVerticalPadding * 2) / dh;
                }

                if (dw <= width - mHorizontalPadding * 2
                        && dh <= height - mVerticalPadding * 2) {// 如果图片的高度<截图区高度，并且图片宽度<截图区宽度时
                ...
                mScaleMatrix.postScale(scale, scale, width / 2, height / 2);// 设置缩放比例
                // 图片移动至屏幕中心
                setImageMatrix(mScaleMatrix);
                once = false;
            }
        ...
        public Bitmap clip() {
            ...
            draw(canvas);
            return Bitmap.createBitmap(bitmap, mHorizontalPadding, mVerticalPadding, width - 2 * mHorizontalPadding,
